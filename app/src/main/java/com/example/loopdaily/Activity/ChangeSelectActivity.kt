...
class ChangeSelectActivity : AppCompatActivity(){
    ...
    lateinit var actionEventDB: ActionEventDB
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        actionEventDB = LitePal.find(ActionEventDB::class.java,id.toLong())
        initTimePicker()
        initView()
    ...
    fun initView(){
        Thread(Runnable {
            ...
            Handler(this.mainLooper).post(Runnable {
                Utils.getInstance(this).GetImg(getString(R.string.server_host)+"/upload/actionDefault/"+actionEventDB.remoteId+"/"+action.imageName,img)
        ...
        button.setOnClickListener {
            if (selectDate==null){
              ...
              LitePal.delete(ActionDB::class.java,actionEventDB.actionId.toLong())
                LitePal.deleteAll(ActionEventDB::class.java,"startDay = ? and actionId = ?",""+actionEventDB.startDay.time,""+actionEventDB.actionId)

                val actionDB = ActionDB(
                    LitePal.count(ActionDB::class.java).toLong(),action.name, selectDate!!,loopTime.time,action.type,true,getString(
                        R.string.server_host)+"/upload/actionDefault/"+action.id+"/"+action.imageName
                    ,action.id)
                if (actionDB.save()){
